...
public final class Moteur implements Runnable {
    ...
    private String[] options(String[] args) {
        ...
        while (args[indice].startsWith("-") || args[indice].startsWith("+")) {
            ...
            if (option.equalsIgnoreCase("tracer")) {
                tracer = true;
            } else if (option.equalsIgnoreCase("etat")) {
                etat = true;
            } else {
    ...
    private void executer(int trait) throws RuntimeErrorException {
        ...
        switch (mnemonic) {
            case MOV:
                mov(trait, ip, arg1, mod1, arg2, mod2);
                break;
            case ADD:
                add(trait, ip, arg1, mod1, arg2, mod2);
                break;
            case SUB:
                sub(trait, ip, arg1, mod1, arg2, mod2);
                break;
            case JMP:
                jmp(trait, ip, arg1, mod1);
                break;
            case JMZ:
                jmz(trait, ip, arg1, mod1, arg2, mod2);
                break;
            case JMG:
                jmg(trait, ip, arg1, mod1, arg2, mod2);
                break;
            case DJZ:
                djz(trait, ip, arg1, mod1, arg2, mod2);
                break;
            case CMP:
                cmp(trait, ip, arg1, mod1, arg2, mod2);
                break;
    ...
    private int mnemonic(int code) {
        return ((code >> 28) & 0xF);
    }

    // extrait l'argument 1
    private int arg1(int code) {
        return ((code >> 14) & 0xFFF);
    }

    // extrait le mode 1
    private int mod1(int code) {
        return ((code >> 26) & 0x3);
    }

    // extrait l'argument 2
    private int arg2(int code) {
        return (code & 0xFFF);
    }

    // extrait le mode 2
    private int mod2(int code) {
        return ((code >> 12) & 0x3);
    }
    ...
    private int valeur(int trait, int ip, int arg, int mod)
        ...
        // mode d'adressage relatif
            ...
            if (mnemonic(memoire[adresse]) != DAT)
                throw new RuntimeErrorException("Erreur arithmetique", noms[trait], ip, cycle);
        ...
        // mode d'adressage indirect
            ...
            int adresse = (ip + arg) % TAILLE;
            if (mnemonic(memoire[adresse]) != DAT)
                throw new RuntimeErrorException("Erreur d'indirection", noms[trait], ip, cycle);
    ...
    private void traceInit() {
        ...
        for (int i = 0; i < nombre; i++)
            ligne.append(completer(noms[i], 24));
    ...
    private String desassemblerMnemonic(int mnemonic) {
        if (mnemonic == DAT)
            ...
            return "JMP";
        else if (mnemonic == JMZ)
            return "JMZ";
        else if (mnemonic == JMG)
            return "JMG";
        else if (mnemonic == DJZ)
            return "DJZ";
    ...
    public int getIP(int couleur) {
        return IP[couleur];
    }

    // renvoie le contenu d'une adresse mémoire
    public int getMem(int adresse) {
        return memoire[adresse];
    }
}
