...
WindowListener,ActionListener,TextListener,Runnable,FilenameFilter
{
	...
	private GraphButton boutonExecuter=
		new GraphButton("Exécuter","img/run.gif");
	...
	{
		if(args.length==0) editeur=new Editeur();
	...
	{
		String chaine=
			completer(Integer.toHexString(texte.getBackground().getRed()))+
  ...
  public void run()
  {
	...
	public void actionPerformed(ActionEvent evt)
	{
		...
		else if(cmd.equals("Enregistrer")) {enregistrer();}
		...
		else if(cmd.equals("Couper")) {couper();}
		else if(cmd.equals("Copier")) {copier();}
		else if(cmd.equals("Coller")) {coller();}
	...
	{
		if(chargement) chargement=false;
		else
		{
			if(!modifie)
			{
